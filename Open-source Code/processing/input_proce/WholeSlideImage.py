...
from utilss.file_utils import load_pkl, save_pkl
class WholeSlideImage(object):
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        if len(results)>1:
            ...
            for idx, (x, y) in enumerate(results):
                patch = self.get_patch(x, y, patch_level, patch_size)
                patch_filename = os.path.join(save_path, f'{self.name}_patch_{idx}.png')
    ...
    def get_patch(self, x, y, patch_level, patch_size):
        patch_downsample = (int(self.level_downsamples[patch_level][0]), int(self.level_downsamples[patch_level][1]))
        ref_patch_size = (patch_size * patch_downsample[0], patch_size * patch_downsample[1])
        patch = np.zeros((ref_patch_size[1], ref_patch_size[0], 3), dtype=np.uint8)
        return patch
